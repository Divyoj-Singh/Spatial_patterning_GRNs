...
plotName = "Long Repressilator"
plotParent = os.getcwd() + "/Plots"
snapit = False # Snapshots for repressilator
saveit = True # Save the simulation?
watch_it = True # Watch the simulation?
...
# Saving it
if(saveit): helpers.save_adv(plotParent, plotName, sol_p, {"sheet":pset_class, "pset_id":ds_i, "diff_coeff":D,}, snapshots = snapit, save_anim=False)
...
plt.hist(pde_dist)
plt.show()
print(pd.Series(pde_dist).value_counts().to_clipboard(excel=True))
