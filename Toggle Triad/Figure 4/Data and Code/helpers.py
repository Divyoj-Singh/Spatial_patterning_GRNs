...
def save_image(img, savepath):
    ...
    plt.clf()
    # Render image
    plt.imshow(img, cmap="jet", interpolation="gaussian", vmin=min(g/k), vmax=max(g/k))
    plt.colorbar()
...
def save_adv(savePath, plotName, sol_p, pset_deets, snapshots, save_anim):
    ...
    if(snapshots):
        ...
        for t in np.arange(0, len(sol_p[0])//5, 20):
            # For each morphogen
            for m in range(3):
                save_with_name = f"{folderName}/Morphogen {m+1} @ {t}.svg"
                save_image(sol_p[m][t], save_with_name)
    else:
        # Saving it
        data = open((folderName+"/data.txt"), "a")
        for i in range(3):
            save_with_name = f"{folderName}/Morphogen {i+1}.svg"
            save_image(sol_p[i][-1], save_with_name)
